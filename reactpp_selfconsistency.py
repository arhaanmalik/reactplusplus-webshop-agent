...
def llm(prompt,stop=["\n"], max_tokens=100, num_responses=10):
    ...
    if num_responses > 1:
        return responses, step_logits
    else:
        return responses[0], step_logits
...
from self_consistecy import xyz_function
...
def webshop_run(idx, prompt, to_print=True):
  ...
  for i in range(15):
    ...
    responses, step_logoits  = llm(init_prompt + prompt[-(6400-len(init_prompt)):], stop=['\n'], max_tokens=100, num_responses=10)
    ...
    action = xyz_function(responses)
    print(f"Final action is: {action}\n\n")
